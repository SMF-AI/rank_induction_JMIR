...
from camelyon.data_models import BinaryLabels
from camelyon.metrics import (
    AverageMeter,
    MetricsMeter,
    plot_auroc,
    plot_prauc,
...
from camelyon.log_ops import save_and_log_figure
...
class CLAMTrainer(BinaryClassifierTrainer):
    ...
    def run_epoch(
        ...
        for step, batch in enumerate(dataloader, start=1):
            xs, ys = batch
            data = xs.squeeze(0).to(self.device)
            label = ys.long().to(self.device)
